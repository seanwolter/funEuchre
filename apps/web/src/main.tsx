...
type RouteKey = "lobby" | "game" | "help";

type WebProtocolContract = {
  outbound: ClientToServerEvent;
  inbound: ServerToClientEvent;
};
...
const ROUTES: Record<RouteKey, RouteView> = {
  lobby: {
    badge: "Phase 1 Shell",
    title: "Lobby Flow Baseline",
    subtitle:
      "Create and join private games fast with clear next steps before cards are dealt.",
    bullets: [
    ...
    panelBody:
      "This shell models the lobby information layout only. Real-time updates and seat state wiring land in later phases."
  },
  game: {
    badge: "Gameplay Surface",
    title: "Authoritative State Projection",
    subtitle:
      "Client UI renders server decisions only and highlights whose turn it is with clear context.",
    bullets: [
    ...
    panelTitle: "Current Focus",
    panelBody:
      "This shell reserves space for gameplay components and routing without implementing card logic yet."
  },
  help: {
    badge: "Onboarding",
    title: "Euchre Quick Rules",
    subtitle:
      "Short reminders for trump, bowers, and scoring to keep first-time players moving.",
    bullets: [
    ...
    panelTitle: "Current Focus",
    panelBody:
      "Rules here mirror project defaults and give players enough context without leaving the game screen."
  }
};

const DEFAULT_ROUTE: RouteKey = "lobby";
const ROUTE_HASHES: ReadonlyArray<RouteKey> = ["lobby", "game", "help"];
...
function resolveRoute(hash: string): RouteKey {
  ...
  if (ROUTE_HASHES.includes(cleaned as RouteKey)) {
    return cleaned as RouteKey;
  }

  return DEFAULT_ROUTE;
}

function routeTab(route: RouteKey, activeRoute: RouteKey): string {
...
function render(route: RouteKey): void {
